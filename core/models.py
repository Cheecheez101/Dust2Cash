...
class Transaction(models.Model):
    ...
    agent = models.ForeignKey(AgentProfile, on_delete=models.SET_NULL, null=True, blank=True, related_name='transactions')
    
    platform = models.CharField(max_length=20, choices=PLATFORM_CHOICES)
    currency = models.CharField(max_length=20, choices=CURRENCY_CHOICES)
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    
    exchange_rate = models.DecimalField(max_digits=10, decimal_places=2, default=100.00)
    ...
    def calculate_amount_to_receive(self):
        total = (self.amount * self.exchange_rate) - self.transaction_fee
